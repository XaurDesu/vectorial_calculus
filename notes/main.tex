...
\def\mytitle{ Calc III } % Title of the ILM project
...
\paragraph*{given $\rho(x,y,z) = x+y+z; \sigma(t) = (\sin(t), \cos(t), t); 0 \leq t \leq 2\pi$; find $m(\sigma) = \int_\sigma \rho ds$.}

\begin{gather}
    m(\sigma) = \int_0^{2\pi} (\sin(t) + \cos(t) + t) \sqrt{2} dt\\
\end{gather}
...
\section{Intertial Momentum}

It evaluates the response of a body to the attempt to rotate it.

$I = {\int\int\int}_W d^2 \rho dV$
...
Stokes' Theorem works a bit like Green's, but concerned with surfaces.
...
$\vec{F} \implies \vec{\nabla} \vec{F} = 0$ and
$\int_{x,y,z}^{x,y,z} \vec{F} dr$ is independent of trajectory.
...
We can now use Stokes' Theorem to affirm that:

\begin{gather}
    {\int \int}_S
\end{gather}

\subsection{Equivalent conditions for a vectorial field.}
...
\paragraph*{Integrate the area of a given surface;}
\begin{equation}
    \begin{cases}
        x = r \cdot \cos{\theta}\\
        y = 2r \cdot \cos{\theta}\\
        z = \theta    
    \end{cases}; \\ 0 \leq r \leq 1 \\
    0 \leq \theta \leq 2\pi
\end{equation}

From this, we can define the surface as:

\begin{gather}
    T(r, \theta) = (r \cos{\theta}, 2r\cos{\theta},\theta)\\
    <\theta = 0>\\
    T(r, \theta) = (r \cos{0}, 2r\cos{0},0)\\
    \alpha(r) = (r, 2r, 0)\\
    <r = t>\\
    \alpha(t) =  (1,2,0) t\\  
\end{gather}

We can also get from other instantiations different values that might help us define the rest of the surface, such as:

\begin{gather}
    <r = 0>\\
    T(r, \theta) = (0 \cos{\theta}, 2\cdot 0\cos{\theta},\theta)\\
    \beta(\theta) = (0,0,1)\theta
\end{gather}

Now, let's take the non-zero extremes:

\begin{gather}
    <\theta = 2\pi>\\
    (x,y,z) = (r,2r,2\pi) = \begin{pmatrix}
        1\\2\\0
...
\begin{gather}
    \int \int_D 1 \cdot ||T_r x T_\theta|| dr d\theta\\
    \int_0^{2\pi} \int_0^1 1 \cdot \sqrt{5 \cdot \cos^2{\theta}} dr d\theta\\
    \int_0^1 dr \cdot \int_0^{2\pi} |\cos{\theta}| d\theta \cdot \sqrt{5}\\
    \sqrt{5} \int_0^{2\pi} |\cos{\theta}| d\theta\\
    4\sqrt{5} (\sin{\theta})|_0^{\frac{\pi}{2}}\\
    4 \sqrt{5}
\end{gather}
